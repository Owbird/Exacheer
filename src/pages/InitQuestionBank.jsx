...
export default function InitQuestionBank() {
    ...
    const _savebank = async (values) => {
        const user = cookies.user
        const id = uuid()

        await saveBank({ ...values, user: user.email, id })

        navigator(`/questions/${id}/add`)

    }

    return (
        <>
            <Title title="Create bank" />
            <Flex
                minH={'100vh'}
                align={'center'}
                justify={'center'}
                ...
                <Stack spacing={8} mx={'auto'} maxW={'lg'} py={12} px={6}>
                    <Stack align={'center'}>
                        <Text fontSize={'lg'} color={'gray.600'}>
                            Let's create a new question bank!
                        </Text>
                    </Stack>
                    <Box
                        rounded={'lg'}
                        ...
                        p={8}>
                        <Formik
                            initialValues={{ title: "", isPublic: true }}
                            onSubmit={async (values, { setSubmitting }) => await _savebank(values).then(() => setSubmitting(false))}
                        >
